   ...
   <pattern id="A-div-quarter">
      ...
      <rule context="tan:equate-works">
         ...
         <report test="exists($sources-equated-multiply)" role="warning"
               ><!-- Items should not be equated more than once --><value-of
               select="$errors//*[@xml:id = 'equ01']"/></report>
         <report test="exists($sources-equated-redundantly)"
               ><!-- Items that share the same IRI value should not be equated. --><value-of
   ...
   <pattern id="A-div-full">
      ...
      <rule context="tan:align | tan:realign">
         ...
         <report test="exists($distro-errors)" tan:applies-to="distribute">When @distribute is
            invoked each source must have the same number of single references (unmatched: <value-of
               select="
            ...
            exceed the range allowed (<value-of
               select="
                  ...
                  return
                     concat('[', string-join(($i/@src, $i/@ref, 'seg', $i/@seg), ' '), ']')"
            /></report>
         <report test="self::tan:realign and exists($repeated-anchors)"
