...
<schema xmlns="http://purl.oclc.org/dsdl/schematron"
   ...
   <pattern>
      ...
      <rule context="tan:feature">
         ...
         <let name="langs-not-supported-by-inclusion"
            value="
               for $i in $these-inclusions,
                  $j in index-of($feature-incl-refs, $i),
                  $k in $these-langs
               return
                  if ($incl-1st-da-resolved[$j]//tan:for-lang[. = $k]) then
                     ()
                  else
                     $k"
         />
      ...
      <rule context="@feature-qty-test">
         <let name="ff" value="../@context"/>
         <let name="this" value="number(.)"/>
         <!--<assert test="exists($ff)">@feature-qty-test requires @context.</assert>-->
