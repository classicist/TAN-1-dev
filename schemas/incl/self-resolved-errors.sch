...
<pattern xmlns="http://purl.oclc.org/dsdl/schematron" xmlns:tan="tag:textalign.net,2015:ns"
   ...
   xmlns:xsl="http://www.w3.org/1999/XSL/Transform" id="self-resolved">
   <title>Errors that are marked in the process of creating $self-core-errors-marked</title>
   <rule context="*">
      ...
      <let name="this-checked-for-errors"
         value="tan:get-via-q-ref($this-q-ref, $self-core-errors-marked)"/>
      ...
      <report test="exists($relevant-warnings)" role="warning">[<value-of select="$relevant-warnings/@xml:id"/>]
         <value-of select="$relevant-warnings/tan:message"/></report>
   ...
   <sqf:fixes>
      <sqf:group id="errors" use-when="$has-include-or-which-attr = false()">
         <sqf:fix id="replace-text"
            use-when="$has-include-or-which-attr = false() and exists($relevant-errors[@xml:id = ('tan04', 'tan10')])">
            <sqf:description>
               <sqf:title>Replace text with: <value-of
                     select="$relevant-errors[@xml:id = ('tan04', 'tan10')]/tan:fix"/></sqf:title>
            </sqf:description>
            <sqf:replace match="text()" select="$relevant-errors[@xml:id = ('tan04', 'tan10')]/tan:fix/text()"
            />
         </sqf:fix>
